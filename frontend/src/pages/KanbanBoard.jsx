...
function KanbanBoard({ workspaceId }) {
  ...
  useEffect(() => {
    ...
    const fetchTasks = async () => {
      try {
        ...
        setTasks((prevTasks) => {
          ...
          const hasChanged =
            prevTasks.length !== updatedTasks.length ||
            prevTasks.some(
              (task, idx) =>
                task._id !== updatedTasks[idx]?._id ||
                JSON.stringify(task) !== JSON.stringify(updatedTasks[idx])
            );
    ...
    const intervalId = setInterval(fetchTasks, 10000); // every 10 seconds
  ...
  const handleCreateTask = async (assignedToId) => {
    const token = localStorage.getItem("token");
    ...
    try {
      await axios.post(
        `${API_BASE}/tasks`,
        { title: newTaskTitle, workspaceId, assignedTo: assignedToId || null },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNewTaskTitle("");
    } catch (err) {
      ...
      {user && user.role === "admin" && (
        <div className="mb-8 p-6 bg-gray-50 rounded-lg shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            />
            <select
              className="w-full sm:w-auto p-3 border rounded-lg focus:ring-blue-500"
              onChange={(e) => {
              ...
              value=""
            >
              <option value="">Assign and Create...</option>
              {members.map((member) => (
